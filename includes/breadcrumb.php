...
function _breadcrumbs()
{
  ...
  /* === OPTIONS === */
  $text['home']     = __('Home',  '_bourbon' ); // text for the 'Home' link
  $text['blog']     = __('Articles',  '_bourbon' ); // text for the 'Home' link
  ...
  if (is_home() || is_front_page()) {
    ...
    if (is_category()) {
      ...
      if (get_query_var('paged')) {
        ...
        if ($show_current == 1) {
          echo $delimiter . $before . $term->name . $after;
        }
      ...
      echo sprintf($link, get_permalink( get_option( 'page_for_posts' ) ),  $text['blog'] ) . $delimiter;
